...
const ContactPage = ({ contact }) => {
  ...
  const handleSubmit = async (e) => {
   ...
   useEffect(() => {
          ...
          const getBannerImages = () => {
            switch (viewport) {
              case "mobile":
                return contact?.data?.bannerUrls?.mobile || [];
              case "tablet":
                return contact?.data?.bannerUrls?.tablet || [];
              case "desktop":
              default:
                return contact?.data?.bannerUrls?.desktop || [];
            }
          };
  ...
  return (
    <>
      ...
      <div className={styles['contact-container-heading']}>
         <h2>Contact Us</h2> 
      </div>
